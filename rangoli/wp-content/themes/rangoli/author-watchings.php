<div class="row">
        ...
        foreach ($user_favs as $user_fav) {
            ...
            if ($post->post_type == "post") {
                if (has_category("look", $post) && isset($user_fav)) {
                    ...
                    echo '<p class="align-center author_posts_heading">' . $name . '\'S WATCHINGS</p>';
                    break;
                }
            }
        }
    ...
    if ($user_favs) {
    ?>
    <div class="flexslider" id="author_look">
        <ul class="slides" >
            <?php
...
<style>
    ...
    #author_look .flex-direction-nav {
        display: block;
        height: 24px;
        position: absolute;
        top: -37px;
    ...
    #author_look .flex-direction-nav .flex-prev {
    ...
    #author_look .flex-direction-nav .flex-next {
        background: url("<?php  echo get_site_url() ?>/wp-content/themes/rangoli/images/arrows-slider.png") repeat scroll 0 0 / 50px auto rgba(0, 0, 0, 0);
        height: 17px;
        right: 0;
