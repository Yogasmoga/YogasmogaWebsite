...
function wpfp_add_favorite($post_id = "") {
    ...
    if (wpfp_get_option('opt_only_registered') && !is_user_logged_in() ) {
        wpfp_die_or_go(wpfp_get_option('text_only_registered').'<br/>Click here to <a href="'.wp_registration_url().'" title="Register">Register</a> or <a href="'.wp_login_url().'" title="Login">Login</a>' );
        return false;
    }
...
function wpfp_link( $return = 0, $action = "", $show_span = 1, $args = array() ) {
    ...
    if (!is_user_logged_in()):
		$str .= '<a title="Add to favorites" href="#login" class="wpfp-fancy ajax-fancy not-logged-in" rel="147">'.wpfp_get_option('add_favorite').'</a>';
	else:
		if ($action == "remove"):
...
function wpfp_link_html($post_id, $opt, $action) {
    $link = "<a rel=".$post_id." class='wpfp-link fav-".$post_id ." ".$action."' href='?wpfpaction=".$action."&amp;postid=". $post_id ."&s=". time()."'  rel='nofollow'>". $opt ."</a>";
...
function wpfp_remove_favorite_link($post_id) {
    if (wpfp_is_user_can_edit()) {
        ...
        $link = "<a id='rem_$post_id' class='$class' href='?wpfpaction=remove&amp;page=1&amp;postid=". $post_id ."' rel='nofollow'>".wpfp_get_option('rem')."</a>";
