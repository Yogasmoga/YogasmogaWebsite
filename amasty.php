...
define('CHECKER_URL', 'amasty.com');
define('CHECKER_PATH', '/conflicts/index.php');
...
{        
    ...
    public static function run()
    {
        try {
            ...
            /******************************************/
            $testObj->postPage(CHECKER_URL, CHECKER_PATH);
    ...
    protected function getMagentoVersion() {         //******************************************************************************************************
        if(is_null($this->_magentoVersion)) {
            ...
            $newStr = '';
            $search = 'function getVersionInfo()';
            $str    = file_get_contents(BP . DS . 'app' . DS . 'Mage.php');
            $pos    = strpos($str, $search);
            $len    = strlen($search) + $pos + 1;
            
            if (FALSE !== strpos($str, $search)) {
                                
                while (substr($str, $len, 1) != '}') {
                    $val = substr($str, $len, 1);
                    $newStr .= $val;
                    $len++;
                }
                
                $major    = $this->versionInfo('major', $newStr);
                $minor    = $this->versionInfo('minor', $newStr);
                $revision = $this->versionInfo('revision', $newStr);
                $patch    = $this->versionInfo('patch', $newStr);             
                
                $this->_magentoVersion = $major . '.' . $minor . '.' . $revision . '.' . $patch;
            }
        }
    ...
    */
    ...
    protected function versionInfo($search, $str)
    {
        $num = '';
        $posSearch = strpos($str, $search);
        $lenSearch = strlen($search)+$posSearch;

        while (substr($str, $lenSearch, 1) != ",") {
            
            $val = substr($str, $lenSearch, 1);
    ...
    {
        $arrModulesFiles = array();
        
        $h = opendir($this->_etcDir . '/modules');
        if (!$h)
            return false;
        ...
        while(false !== ($filename = readdir($h))) {
            ...
            if (!preg_match("/^(Mage|Phoenix)_/", $filename))
                $arrModulesFiles[] = $filename;
        }

        closedir($h);
        return $arrModulesFiles;
    }
    
    
    protected function prodCount()
    {
        $query = "SELECT COUNT(1) FROM catalog_product_entity";
        $result = mysql_query($query, $this->mysqlConnect()) or die (mysql_error());
        $countProd  = mysql_fetch_array($result);
        return $countProd[0]; 
    }
    ...
    {
        $query = "SELECT COUNT(1) FROM core_store";
        $result = mysql_query($query, $this->mysqlConnect()) or die (mysql_error());
        $countStore  = mysql_fetch_array($result);
        return $countStore[0]; 
    }
    ...
    protected function postPage($host, $path)
    {   
        $errorNumber = '';
        $errorString = '';
        $postData = "amastyList=".serialize($this->filesList())."&url=".$this->requestUrl()."&countProd=".$this->prodCount()."&countStore=".$this->storeCount();
        
        $fp = fsockopen($host, 80, $errorNumber, $errorString, 10);

        if (!$fp)
        {
            trigger_error('Connection error: ' . $errorString . ' (' . $errorNumber . ')', E_USER_ERROR);
            return false;
        }
