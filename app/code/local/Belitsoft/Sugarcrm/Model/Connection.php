...
#		Mage::log('OPERATION: '.$operation);
		...
		$beans = Mage::getModel('sugarcrm/operations')->getEnabledOperations($operation);
			...
			if($operation == self::OPERATION_DELETE) {
				...
				foreach($fieldsmap_items as $key=>$value) {
					...
					if(($fields_mapping_type == self::SYNC_MAGEFIELD) && $mage_customer_field && is_array($customer_data)) {
						...
						$sugarcrm_value = self::evalCode($eval_code, $customer_data, $sugarcrm_field, $bean_name);
					}
...
#			Varien_Profiler::stop("SUGARCRM: connection_synch_order");
		...
		//echo "<pre>"; var_dump(Mage::getSingleton('checkout/session')); die;		
		...
		if($operation != self::OPERATION_DELETE) {
			...
			if($bean_name == self::CASES) {
				...
				/*
					...
					if($order_state == Mage_Sales_Model_Order::STATE_CLOSED) {
						$values[] = array('name' => self::STATUS, 'value' => self::CASES_STATUS_CLOSED);
					} else if($order_state == Mage_Sales_Model_Order::STATE_NEW) {
						$values[] = array('name' => self::STATUS, 'value' => self::CASES_STATUS_NEW);
					} else if($order_state == Mage_Sales_Model_Order::STATE_CANCELED) {
						$values[] = array('name' => self::STATUS, 'value' => self::CASES_STATUS_CANCELED);
					}
				}*/
				...
				/*
					...
					if(($order_state == Mage_Sales_Model_Order::STATE_CLOSED) || ($order_state == Mage_Sales_Model_Order::STATE_COMPLETE)) {
						$values[] = array('name' => self::SALES_STAGE, 'value' => self::OPPORTUNITY_STATUS_CLOSED);
					} else if($order_state == Mage_Sales_Model_Order::STATE_NEW) {
						$values[] = array('name' => self::SALES_STAGE, 'value' => self::OPPORTUNITY_STATUS_NEW);
					} else if($order_state == Mage_Sales_Model_Order::STATE_CANCELED) {
						$values[] = array('name' => self::SALES_STAGE, 'value' => self::OPPORTUNITY_STATUS_CANCELED);
					}
				}*/
...
/*
			$relationParameters = array(
				...
				'module_name' => 'Opportunities',  // your module name
				'module_id' => "$opportunityId", // your first module id
				'link_field_name' => 'ap_ap_products_opportunities', // your second module
				'related_ids' => array("$productId"), // send module id
			);
...
#		Varien_Profiler::stop("SUGARCRM: connection_synch_all_quotes");
	...
	static function evalCode($eval_code, $customer_data, $sugarcrm_field, $bean_name)
	{
		$return_object = '';
