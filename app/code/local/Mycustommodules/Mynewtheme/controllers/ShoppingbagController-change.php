...
//                            <span class="f-right capstxt">'."$".number_format((float)($tax), 2, '.', '').'</span></li>';
        ...
        $customerId = Mage::getModel('customer/session')->getCustomerId();
        if(!$customerId)
            $html .=' <label><input type="text" name="smogi" class="gry lightgray" available="0" id="smogi" value="Sign in to use SMOGI Bucks" disabled="disabled"/><span  class="smogi-login">Apply</span></label>
                        <label><input type="text" name="giftcartcode" data-used="no" class="gry lightgray" id="giftcartcode" value="Sign in to use Promo Code / Gift Card Code" disabled="disabled" /><span class="giftcardlogin">Apply</span></label>';
        else{
            $gryclasssmogi = "";
            $gryclasspromo = "";
            $gryclassgift = "";
