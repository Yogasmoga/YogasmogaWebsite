(function () {
	jQuery.widget("ui.rcarousel", {
		_create: function() {
			var data,
				jQueryroot = jQuery( this.element ),
				_self = this,
				options = this.options;

			// if options were default there should be no problem
            // check if user set options before init: jQuery('element').rcarousel({with: "foo", visible: 3});
            // in above example exception will be thrown because 'with' should be a number!
            this._checkOptionsValidity( this.options );

			// for every carousel create a data object and keeps it in the element
			this._createDataObject();
			data = jQueryroot.data( "data" );
			...
			jQuery( options.navigation.next ).click(
				function( event ) {
					_self.next();
			...
			jQuery( options.navigation.prev ).click(
				function( event ) {
					_self.prev();
					event.preventDefault();
				}
			...
			data.navigation.next = jQuery( options.navigation.next );
			data.navigation.prev = jQuery( options.navigation.prev );
		...
		_addElement: function( jQueryElement, direction ) {
			var jQueryroot = jQuery( this.element ),
				jQuerycontent = jQueryroot.find( "div.wrapper" ),
				options = this.options;
			...
			if ( options.orientation === "horizontal" ) {
				jQuery( jQueryElement ).css( "marginRight", options.margin );
			} else {
				jQuery( jQueryElement ).css({
			...
			if ( direction === "prev" ) {
				...
				jQuerycontent.prepend( jQueryElement.clone(true, true) );
			} else {
				jQuerycontent.append( jQueryElement.clone(true, true) );
			}			
		},
		...
		_autoMode: function( direction ) {
			var options = this.options,
				data = jQuery( this.element ).data( "data" );

			if ( direction === "next" ) {
				data.interval = setTimeout( jQuery.proxy(this.next, this), options.auto.interval );
			} else {
				data.interval = setTimeout( jQuery.proxy(this.prev, this), options.auto.interval );
			}
		},
			...
			jQuery.each(options,
				function( key, value ) {
					...
					switch ( key ) {
						...
						case "navigation":
							if ( !value || jQuery.isPlainObject(value) === false ) {
		...
		getCurrentPage: function() {
			var data = jQuery( this.element ).data( "data" );
		...
		_goToPrevPage: function( by ) {
			var _page, _oldPage, _dist, _index, _animOpts, jQuerylastEl, _unique, _pos, _theSame,
				jQueryroot = jQuery( this.element ),
				self = this,
				options = this.options,
				data = jQuery( this.element ).data( "data" );
			...
			jQuery( _page ).each(
				function( i, el ) {
					if ( el.get(0) === jQuery(_oldPage[i]).get(0) ) {
						_theSame = true;
					} else {
						_theSame = false;
					}
			...
			jQuerylastEl = _page[_page.length - 1].get( 0 );
			for ( var i = _oldPage.length - 1; i >= 0; i-- ) {
				if ( jQuerylastEl === jQuery(_oldPage[i]).get(0) ) {
					_unique = false;
					_pos = i;
					break;
				} else {
			...
			if (options.orientation === "horizontal") {
				...
				jQueryroot.scrollLeft( _dist );
			} else {
				_animOpts = {scrollTop: 0};
				jQueryroot.scrollTop( _dist );
			}
		...
		_goToNextPage: function( by ) {
			var _page, _oldPage, _dist, _index, _animOpts, jQueryfirstEl, _unique, _pos, _theSame,
				jQueryroot = jQuery( this.element ),
				options = this.options,
				data = jQueryroot.data( "data" ),				
				self = this;
			...
			// remaining were loaded already
			jQueryfirstEl = _page[0].get( 0 );
			for ( var i = 0; i < _page.length; i++) {
				if ( jQueryfirstEl === jQuery(_oldPage[i]).get(0) ) {
					_unique = false;
					_pos = i;
					break;
				} else {
			...
			jQueryroot
				.animate(_animOpts, options.speed, function() {
					...
					if ( options.orientation === "horizontal" ) {
						jQueryroot.scrollLeft( 0 );
					} else {
						jQueryroot.scrollTop( 0 );
					}
		...
		_saveElements: function() {
			var jQueryel,
				jQueryroot = jQuery( this.element ),
				jQueryelements = jQueryroot.find( "div.wrapper" ).children(),
				data = jQueryroot.data( "data" );
			...
			jQueryelements.each(
				function( i, el ) {
					...
					data.paths.push( jQueryel.clone(true, true) );
					jQueryel.remove();
				}
			);		
		},
		...
		_setOption: function( key, value ) {
			...
			switch (key) {
				...
				case "auto":
					_newOptions = jQuery.extend( options.auto, value );
